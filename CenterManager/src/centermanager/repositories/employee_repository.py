...
class EmployeeRepository(BaseRepository[Employee]):
    ...
    def count_non_archived(self) -> int:
        return self._session.query(Employee).filter(Employee.employment_status != Employee.STATUS_ARCHIVED).count()

    def count_by_status(self, status: str) -> int:
        return self._session.query(Employee).filter(Employee.employment_status == status).count()
    ...
    def operational_history_counts(self, employee_id: int) -> dict[str, int]:
