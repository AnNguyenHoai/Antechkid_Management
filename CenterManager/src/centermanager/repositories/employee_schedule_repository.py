...
class EmployeeScheduleRepository:
    ...
    def list_rules(self, employee_id: int) -> List[EmployeeScheduleRule]:
        return self._session.query(EmployeeScheduleRule).filter_by(employee_id=employee_id).order_by(EmployeeScheduleRule.day_of_week, EmployeeScheduleRule.start_time, EmployeeScheduleRule.effective_from).all()
    def list_exceptions(self, employee_id: int) -> List[EmployeeScheduleException]:
        return self._session.query(EmployeeScheduleException).filter_by(employee_id=employee_id).order_by(EmployeeScheduleException.schedule_date).all()
    def get_rule(self, rule_id: int) -> Optional[EmployeeScheduleRule]: return self._session.get(EmployeeScheduleRule, rule_id)
    def get_exception(self, exception_id: int) -> Optional[EmployeeScheduleException]: return self._session.get(EmployeeScheduleException, exception_id)
