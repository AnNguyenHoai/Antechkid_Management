...
class EmployeeWorkingTimeRepository:
    ...
    def get(self, entry_id: int) -> Optional[EmployeeWorkingTimeEntry]:
        return self._session.get(EmployeeWorkingTimeEntry, entry_id)

    def list_for_employee(self, employee_id: int, start_date: date | None = None, end_date: date | None = None):
        q = self._session.query(EmployeeWorkingTimeEntry).filter_by(employee_id=employee_id)
    ...
    def open_entry(self, employee_id: int):
        return self._session.query(EmployeeWorkingTimeEntry).filter_by(employee_id=employee_id, status=EmployeeWorkingTimeEntry.STATUS_OPEN).order_by(EmployeeWorkingTimeEntry.id.desc()).first()
