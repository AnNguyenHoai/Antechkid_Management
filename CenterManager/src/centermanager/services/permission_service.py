...
Persistence is delegated to RepositoryProvider; the service owns business
validation and transaction orchestration.
"""
import logging
import secrets
import string
...
class PermissionService:
    ...
    def _audit(self, action: str, target=None, details=None) -> None:
        ...
        try:
            ...
            AuditService(self._session_factory, self._repository_provider).record(
                action, module, target_type, getattr(target, "id", None),
                getattr(target, "name", None) or getattr(target, "username", None),
                details=details,
            )
        except Exception:
    ...
    def is_reception(self, user: Optional[User] = None) -> bool:
        user = user or get_current_user()
        return bool(user and user.is_reception)

    # ===== Role & Permission Management =====

    def get_all_roles(self) -> List[Role]:
        with self._session_factory() as session:
            return self._repository_provider.roles(session).list_all_with_permissions()

    def get_role(self, role_id: int) -> Optional[Role]:
        with self._session_factory() as session:
            return self._repository_provider.roles(session).get_by_id_with_permissions(role_id)

    def get_permissions_by_category(self):
        with self._session_factory() as session:
            permissions = self._repository_provider.permissions(session).list_all()
            grouped = {}
    ...
    def _resolve_permissions(self, session, permission_names: Set[str]) -> List[Permission]:
        known = {p.name: p for p in self._repository_provider.permissions(session).list_all()}
    ...
    def create_role(self, name: str, display_name: str, description: Optional[str], permission_names: Set[str]) -> Role:
        ...
        with self._session_factory() as session:
            repo = self._repository_provider.roles(session)
            ...
            repo.add(role); session.commit(); repo.refresh(role)
    ...
    def update_role(self, role_id: int, display_name: str, description: Optional[str], permission_names: Set[str]) -> Role:
        ...
        with self._session_factory() as session:
            repo = self._repository_provider.roles(session)
            role = repo.get_by_id_with_permissions(role_id)
            if role is None:
            ...
            if role.is_system:
                ...
                role.permissions = self._resolve_permissions(session, permission_names)
            session.commit(); repo.refresh(role)
    ...
    def delete_role(self, role_id: int) -> None:
        with self._session_factory() as session:
            ...
            if repo.count_users(role_id) > 0:
                raise RoleLifecycleError("A role assigned to users cannot be deleted. Reassign its users first.")
            role_name = role.name; target_role_id = role.id
            repo.delete(role); session.commit()
            self._audit("ROLE_DELETED", None, {"role_id": target_role_id, "role": role_name})
    ...
    def _count_active_admins(self, session) -> int:
        admin_role = self._repository_provider.roles(session).get_by_name(RoleDefinitions.ADMIN)
        if admin_role is None:
            return 0
        return self._repository_provider.users(session).count_active_by_role_id(admin_role.id)
    ...
    def get_user(self, user_id: int) -> Optional[User]:
        with self._session_factory() as session:
            return self._repository_provider.users(session).get_by_id_with_role(user_id)

    def get_user_by_username(self, username: str) -> Optional[User]:
        with self._session_factory() as session:
            return self._repository_provider.users(session).get_by_username(username)
    ...
    def get_all_users(self) -> List[User]:
        ...
        with self._session_factory() as session:
            return self._repository_provider.users(session).list_all()
    ...
    def authenticate_user(self, username: str, password: str) -> User:
        ...
        with self._session_factory() as session:
            repo = self._repository_provider.users(session)
    ...
    def update_user_role(self, user_id: int, role_name: str) -> User:
        with self._session_factory() as session:
            ...
            session.commit(); user_repo.refresh(user)
    ...
    def delete_user(self, user_id: int) -> None:
        with self._session_factory() as session:
            ...
            linked_employee = repo.get_employee_linked_to_user(user_id)
    ...
    def create_user_with_temp_password(
        ...
        with self._session_factory() as session:
            ...
            user = User(
                ...
                full_name=full_name, email=email, phone=phone, role_id=role.id,
                is_active=True, force_password_change=True, login_attempts=0,
            )
            user_repo.add(user)
            user_repo.flush()
            ...
            if self._should_provision_employee(role_name):
                ...
                employee = Employee(
                    employee_code=f"EMP-{next_number:05d}", full_name=full_name,
                    phone=phone, email=email, employment_status=Employee.STATUS_ACTIVE,
                    hire_date=None, user_id=user.id,
                )
            ...
            if employee is not None:
                employee_repo.refresh(employee)
                logger.info(
