...
class AuditService:
    ...
    def __init__(self, session_factory, repository_provider: RepositoryProvider):
    ...
    def log_user_action(self, action, user, details=None, **kwargs):
        return self.record(action, "admin", "user", getattr(user, "id", None), getattr(user, "username", None), details=details, **kwargs)

    def log_role_action(self, action, role, details=None, **kwargs):
        return self.record(action, "admin", "role", getattr(role, "id", None), getattr(role, "name", None), details=details, **kwargs)
