...
class AuditService:
    ...
    def record(self, action: str, module: str, target_type: Optional[str]=None, target_id: Optional[Any]=None, target_name: Optional[str]=None, result: str='success', details: Optional[Any]=None, actor=None) -> AuditLog:
        ...
        with self._session_factory() as session:
            log=AuditLog(actor_id=getattr(actor,'id',None), actor_name=getattr(actor,'username',None), action=action, module=module, target_type=target_type, target_id=str(target_id) if target_id is not None else None, target_name=target_name, result=result, details=details)
