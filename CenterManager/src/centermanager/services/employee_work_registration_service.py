...
class EmployeeWorkRegistrationService:
    ...
    def _require_permission(self,p,u):
        ...
        if not allowed:
            logger.error("[WORK_REGISTRATION_PERMISSION_DENIED] permission=%s user_id=%s username=%s role=%s employee_id=%s user_object=%s",p,getattr(u,"id",None),getattr(u,"username",None),getattr(getattr(u,"role",None),"name",None),getattr(getattr(u,"employee",None),"id",None),type(u).__name__,stack_info=True)
    ...
    def _scope(self,employee_id,user=None):
        ...
        with self._sf() as s:
            ...
            if e.user_id==u.id:
                ...
                if not allowed_self: raise EmployeeWorkRegistrationAccessDeniedError(f"Permission '{self.SELF_PERMISSION}' is required.")
    ...
    def _period(self,s,y,m):
        self._month_range(y,m); return self._repository_provider.employee_work_registration_periods(s).get_or_create(y,m)
    ...
    def list_all(self,y,m,user=None):
        ...
        with self._sf() as s:p=self._period(s,y,m);return self._repository_provider.employee_work_registrations(s).list_all(p.id)
    @staticmethod
    def _validate(d,start,end,typ):
