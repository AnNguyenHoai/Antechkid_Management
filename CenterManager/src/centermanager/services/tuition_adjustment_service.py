...
class TuitionAdjustmentService:
    ...
    def __init__(self, session_factory: sessionmaker, repository_provider: Optional[RepositoryProvider] = None,
                 audit_service: Optional[AuditService] = None, event_bus: Optional[EventBus] = None) -> None:
        self._session_factory = session_factory
        self._repository_provider = repository_provider or create_default_repository_provider()
        self._audit_service = audit_service or AuditService(session_factory, repository_provider=self._repository_provider)
    ...
    def _publish(self, adjustment: TuitionAdjustment) -> None:
        if self._event_bus is None:
            return
        self._event_bus.publish(FinanceDataChanged(entity="tuition_adjustment", action=adjustment.kind.lower(), entity_id=adjustment.id))
        if adjustment.linked_income_id is not None:
            self._event_bus.publish(FinanceDataChanged(entity="income", action="created", entity_id=adjustment.linked_income_id))

    def _ensure_date_mutable(self, session, adjustment_date: date):
        try:
            resolved = FinanceLedgerGuard.ensure_date_mutable(session, self._repository_provider, adjustment_date)
            config = self._repository_provider.finance_periods(session).get_unique_effective(adjustment_date)
        except ValueError as exc:
            raise TuitionAdjustmentValidationError(str(exc)) from exc
        if config is None:
            raise TuitionAdjustmentValidationError(f"No accounting configuration covers adjustment date {adjustment_date.isoformat()}.")
    ...
    @staticmethod
    def _same_command(existing: TuitionAdjustment, *, kind: str, enrollment_id: int, amount: Decimal,
                      adjustment_date: date, origin_income_id: Optional[int], origin_adjustment_id: Optional[int],
                      wallet: Optional[str], reason: str) -> bool:
        return (
            existing.kind == kind and existing.enrollment_id == enrollment_id
            and _money(existing.amount) == amount and existing.adjustment_date == adjustment_date
            and existing.origin_income_id == origin_income_id
            and existing.origin_adjustment_id == origin_adjustment_id
            and existing.wallet == wallet and existing.reason == reason
        )
    ...
    def preview(self, enrollment_id: int, *, origin_income_id: Optional[int] = None,
        ...
        with self._session_factory() as session:
            ...
            result = {"enrollment_id": enrollment_id, "refundable_amount": max(settled, Decimal("0.00")), "as_of_date": cutoff}
            if origin_income_id is not None:
                ...
                if (origin is None or origin.income_type != "Tuition" or origin.status != Income.STATUS_ACTIVE
                        or float(origin.amount) <= 0 or origin.enrollment_id != enrollment_id):
                    raise TuitionAdjustmentValidationError("Origin must be a positive ACTIVE Tuition payment for this Enrollment.")
                refunded = _money(adjustments.sum_refunds_for_origin(origin_income_id))
                result["origin_remaining_refundable"] = max(_money(origin.amount) - refunded, Decimal("0.00"))
            return result

    @require_permission("finance.income.create")
    def refund(self, enrollment_id: int, amount, payment_method: str, adjustment_date: date, *, reason: str,
               idempotency_key: str, origin_income_id: Optional[int] = None, prepaid_only: bool = False) -> TuitionAdjustment:
        ...
        command = dict(kind=TuitionAdjustment.KIND_REFUND, enrollment_id=enrollment_id, amount=value,
                       adjustment_date=effective_date, origin_income_id=origin_income_id, origin_adjustment_id=None,
                       wallet=wallet, reason=resolved_reason)
        try:
            with self._session_factory() as session:
                adjustments = self._repository_provider.tuition_adjustments(session)
                ...
                if origin_income_id is not None:
                    ...
                    if origin.enrollment_id != enrollment_id:
                        raise TuitionAdjustmentValidationError("Origin payment belongs to a different Enrollment.")
                    already_refunded = _money(adjustments.sum_refunds_for_origin(origin_income_id))
                    remaining = max(_money(origin.amount) - already_refunded, Decimal("0.00"))
                    if value > remaining:
                        raise TuitionAdjustmentValidationError(f"Refund amount {value} exceeds origin remaining refundable {remaining}.")
                settled = _money(incomes.sum_active_tuition_for_enrollment(enrollment_id, as_of_date=effective_date))
                refundable = max(settled, Decimal("0.00"))
                if value > refundable:
                    raise TuitionAdjustmentValidationError(f"Refund amount {value} exceeds refundable tuition {refundable}.")
                if prepaid_only:
                    ...
                    try:
                        accrual = TuitionAccrualService.calculate_from_records(enrollment, sessions, effective_date)
                    except TuitionAccrualUnresolvedError as exc:
                        raise TuitionAdjustmentValidationError("Enrollment tuition accrual is unresolved.") from exc
                    prepaid = max(_money(settled - Decimal(accrual.net_accrued)), Decimal("0.00"))
                    if value > prepaid:
                        raise TuitionAdjustmentValidationError(f"Prepaid refund {value} exceeds available prepaid credit {prepaid}.")
                accounting_config, accounting_period_start = self._ensure_date_mutable(session, effective_date)
                actor = get_current_user()
                income = Income(student_id=enrollment.student_id, class_id=enrollment.class_id, enrollment_id=enrollment.id,
                                amount=-float(value), income_type="Tuition", payment_method=wallet, payment_date=effective_date,
                                payment_period="REFUND", finance_period_id=accounting_config.id,
                                finance_period_start=accounting_period_start, received_by=self._actor_name(actor),
                                note=f"tuition_adjustment_refund; reason={resolved_reason}", status=Income.STATUS_ACTIVE)
                incomes.add(income)
                incomes.flush()
                adjustment = TuitionAdjustment(enrollment_id=enrollment.id, origin_income_id=origin_income_id,
                                               linked_income_id=income.id, kind=TuitionAdjustment.KIND_REFUND,
                                               amount=value, adjustment_date=effective_date, wallet=wallet,
                                               reason=resolved_reason, idempotency_key=key,
                                               created_by=getattr(actor, "username", None))
                adjustments.add(adjustment)
                adjustments.flush()
                self._audit_service.record_in_session(
                    session, action="TUITION_REFUND", module="tuition", target_type="tuition_adjustment",
                    target_id=adjustment.id, target_name=f"Tuition Refund #{adjustment.id}", actor=actor,
                    details={"enrollment_id": enrollment.id, "origin_income_id": origin_income_id,
                             "linked_income_id": income.id, "amount": str(value), "wallet": wallet,
                             "adjustment_date": effective_date.isoformat(),
                             "accounting_period_start": accounting_period_start.isoformat(),
                             "prepaid_only": prepaid_only, "idempotency_key": key, "reason": resolved_reason},
                    entity_type="TuitionAdjustment", entity_id=adjustment.id,
    ...
    @require_permission("finance.income.create")
    def credit_adjustment(self, enrollment_id: int, amount, adjustment_date: date, *, reason: str,
                          idempotency_key: str, origin_adjustment_id: Optional[int] = None) -> TuitionAdjustment:
        value = self._validate_amount(amount)
        effective_date = self._validate_date(adjustment_date)
        resolved_reason = self._reason(reason)
        key = self._idempotency_key(idempotency_key)
        command = dict(kind=TuitionAdjustment.KIND_CREDIT, enrollment_id=enrollment_id, amount=value,
                       adjustment_date=effective_date, origin_income_id=None, origin_adjustment_id=origin_adjustment_id,
                       wallet=None, reason=resolved_reason)
        try:
            with self._session_factory() as session:
                adjustments = self._repository_provider.tuition_adjustments(session)
                ...
                actor = get_current_user()
                adjustment = TuitionAdjustment(enrollment_id=enrollment.id, origin_adjustment_id=origin_adjustment_id,
                                               linked_income_id=None, kind=TuitionAdjustment.KIND_CREDIT, amount=value,
                                               adjustment_date=effective_date, wallet=None, reason=resolved_reason,
                                               idempotency_key=key, created_by=getattr(actor, "username", None))
                adjustments.add(adjustment)
                adjustments.flush()
                self._audit_service.record_in_session(
                    session, action="TUITION_CREDIT_ADJUSTMENT", module="tuition", target_type="tuition_adjustment",
                    target_id=adjustment.id, target_name=f"Tuition Credit Adjustment #{adjustment.id}", actor=actor,
                    details={"enrollment_id": enrollment.id, "origin_adjustment_id": origin_adjustment_id,
                             "amount": str(value), "adjustment_date": effective_date.isoformat(),
                             "accounting_period_start": accounting_period_start.isoformat(), "idempotency_key": key,
                             "reason": resolved_reason, "wallet_movement": False}, entity_type="TuitionAdjustment",
