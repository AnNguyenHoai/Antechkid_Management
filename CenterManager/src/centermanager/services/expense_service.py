...
class ExpenseService:
    ...
    Accounting contract: Pending records are not realized outflows; Completed
    records are realized. Legacy ``Paid`` rows remain realized through the
    ...
    def list_expenses(
        ...
        with self._session_factory() as session:
            ...
            return (
                repo.list_active(offset=offset, limit=per_page, sort_by=sort_by, ascending=ascending, **kwargs),
    ...
    def export_expenses_csv(
        ...
        """Export every active Expense matching the current server-side filters."""
        ...
        try:
            with os.fdopen(fd, "w", encoding="utf-8-sig", newline="") as handle:
                ...
                for expense in rows:
                    writer.writerow([
                        expense.payment_date.isoformat(), expense.category, expense.description or "",
