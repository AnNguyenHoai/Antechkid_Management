...
class WorkingTimeBookingDialog(QDialog):
    ...
    def values(self):
        return (self.day.date().toPython(), self.start.time().toPython(), self.end.time().toPython(), self.typ.currentText(), self.notes.text().strip() or None)
...
class EmployeeWorkingTimeWidget(QWidget):
    ...
    def _build(self):
        ...
        top=QHBoxLayout(); self.month=QDateEdit(); self.month.setCalendarPopup(True); self.month.setDisplayFormat("MM/yyyy"); self.month.setDate(QDate.currentDate()); self.month.dateChanged.connect(self.refresh); top.addWidget(QLabel("Month")); top.addWidget(self.month)
        self.summary=QLabel("-"); top.addWidget(self.summary,1)
        self.checkin=QPushButton("Check In"); self.checkout=QPushButton("Check Out")
        self.add=QPushButton("Book Time"); self.edit=QPushButton("Edit"); self.delete=QPushButton("Delete")
        for b in (self.checkin,self.checkout): top.addWidget(b)
        if self.management:
            for b in (self.add,self.edit,self.delete): top.addWidget(b)
        root.addLayout(top)
        self.table=QTableWidget(0,7); self.table.setHorizontalHeaderLabels(["Date","Start","End","Hours","Work Type","Source","Status"]); self.table.setMinimumHeight(260); self.table.verticalHeader().setDefaultSectionSize(30); self.table.horizontalHeader().setSectionResizeMode(0,QHeaderView.ResizeMode.ResizeToContents); self.table.horizontalHeader().setSectionResizeMode(1,QHeaderView.ResizeMode.ResizeToContents); self.table.horizontalHeader().setSectionResizeMode(2,QHeaderView.ResizeMode.ResizeToContents); self.table.horizontalHeader().setSectionResizeMode(3,QHeaderView.ResizeMode.ResizeToContents); self.table.horizontalHeader().setSectionResizeMode(4,QHeaderView.ResizeMode.ResizeToContents); self.table.horizontalHeader().setSectionResizeMode(5,QHeaderView.ResizeMode.ResizeToContents); self.table.horizontalHeader().setSectionResizeMode(6,QHeaderView.ResizeMode.Stretch); self.table.setSelectionBehavior(QTableWidget.SelectionBehavior.SelectRows); self.table.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers); root.addWidget(self.table)
    ...
    def refresh(self):
        try:
            ...
            for e in rows:
                ...
                mins=self.service._minutes(e.start_time,e.end_time); actual += mins
        ...
        except Exception as exc: QMessageBox.warning(self,"Working Time",f"Could not load working time.\n\n{exc}")
    def _add(self):
        d=WorkingTimeBookingDialog(self,default_date=date.today())
        if d.exec():
            try:self.service.create_booking(self.employee.id,*d.values());self.refresh()
            except Exception as e:QMessageBox.warning(self,"Working Time",str(e))
    def _edit(self):
        ...
        try:
            ...
            if d.exec():
                vals=d.values(); self.service.update_booking(eid,work_date=vals[0],start_time=vals[1],end_time=vals[2],work_type=vals[3],notes=vals[4]); self.refresh()
        except Exception as e:QMessageBox.warning(self,"Working Time",str(e))
    def _delete(self):
        i=self.table.currentRow()
        if i<0:return
        ...
        if QMessageBox.question(self,"Delete working time","Delete selected working-time entry?")==QMessageBox.StandardButton.Yes:
            try:self.service.delete_entry(eid);self.refresh()
            except Exception as e:QMessageBox.warning(self,"Working Time",str(e))
    def _check_in(self):
        try:self.service.check_in(self.employee.id);self.refresh()
        except Exception as e:QMessageBox.warning(self,"Check In",str(e))
    def _check_out(self):
        try:
            ...
            self.service.check_out(open_rows[0].id); self.refresh()
        except Exception as e:QMessageBox.warning(self,"Check Out",str(e))
