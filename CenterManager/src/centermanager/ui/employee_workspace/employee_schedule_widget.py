...
from PySide6.QtWidgets import QWidget,QVBoxLayout,QHBoxLayout,QTableWidget,QTableWidgetItem,QPushButton,QDialog,QDialogButtonBox,QComboBox,QDateEdit,QTimeEdit,QFormLayout,QLineEdit,QMessageBox,QGroupBox,QCheckBox,QHeaderView
...
class EmployeeScheduleWidget(QWidget):
    ...
    def _build(self):
        ...
        rules_box=QGroupBox("Weekly Schedule"); rl=QVBoxLayout(rules_box); self.rules=QTableWidget(0,5);self.rules.setHorizontalHeaderLabels(["Day","Time","Effective from","Effective to","Notes"]);self.rules.setSelectionBehavior(QTableWidget.SelectionBehavior.SelectRows);self.rules.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers);self.rules.setMinimumHeight(190);self.rules.verticalHeader().setDefaultSectionSize(32);self.rules.horizontalHeader().setSectionResizeMode(0,QHeaderView.ResizeMode.ResizeToContents);self.rules.horizontalHeader().setSectionResizeMode(1,QHeaderView.ResizeMode.ResizeToContents);self.rules.horizontalHeader().setSectionResizeMode(2,QHeaderView.ResizeMode.ResizeToContents);self.rules.horizontalHeader().setSectionResizeMode(3,QHeaderView.ResizeMode.ResizeToContents);self.rules.horizontalHeader().setSectionResizeMode(4,QHeaderView.ResizeMode.Stretch);rl.addWidget(self.rules)
        ...
        ex_box=QGroupBox("Schedule Exceptions");el=QVBoxLayout(ex_box);self.exceptions=QTableWidget(0,4);self.exceptions.setHorizontalHeaderLabels(["Date","Type","Time","Notes"]);self.exceptions.setSelectionBehavior(QTableWidget.SelectionBehavior.SelectRows);self.exceptions.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers);self.exceptions.setMinimumHeight(150);self.exceptions.verticalHeader().setDefaultSectionSize(32);self.exceptions.horizontalHeader().setSectionResizeMode(0,QHeaderView.ResizeMode.ResizeToContents);self.exceptions.horizontalHeader().setSectionResizeMode(1,QHeaderView.ResizeMode.ResizeToContents);self.exceptions.horizontalHeader().setSectionResizeMode(2,QHeaderView.ResizeMode.ResizeToContents);self.exceptions.horizontalHeader().setSectionResizeMode(3,QHeaderView.ResizeMode.Stretch);el.addWidget(self.exceptions);eb=QHBoxLayout();self.add_ex=QPushButton("Add Exception");self.del_ex=QPushButton("Delete");eb.addWidget(self.add_ex);eb.addWidget(self.del_ex);eb.addStretch();el.addLayout(eb);root.addWidget(ex_box)
        ...
        if not self.editable:
            for b in (self.add_rule,self.edit_rule,self.del_rule,self.add_ex,self.del_ex): b.setEnabled(False)
    ...
    def refresh(self):
        ...
        except Exception as exc: QMessageBox.warning(self,"Schedule",f"Could not load schedule.\n\n{exc}")
    def _add_rule(self):
        d=ScheduleRuleDialog(self)
        if d.exec():
            try:self.service.add_rule(self.employee.id,*d.values());self.refresh()
            except Exception as e:QMessageBox.warning(self,"Schedule",str(e))
    def _edit_rule(self):
        ...
        try:
            ...
            if d.exec():self.service.update_rule(rid,day_of_week=d.values()[0],start_time=d.values()[1],end_time=d.values()[2],effective_from=d.values()[3],effective_to=d.values()[4],notes=d.values()[5]);self.refresh()
        except Exception as e:QMessageBox.warning(self,"Schedule",str(e))
    def _delete_rule(self):
        i=self.rules.currentRow()
        if i<0:return
        rid=self.rules.item(i,0).data(Qt.ItemDataRole.UserRole)
        if QMessageBox.question(self,"Delete schedule","Delete selected schedule rule?")==QMessageBox.StandardButton.Yes:
            try:self.service.delete_rule(rid);self.refresh()
            except Exception as e:QMessageBox.warning(self,"Schedule",str(e))
    def _add_exception(self):
        d=ScheduleExceptionDialog(self)
        if d.exec():
            try:self.service.add_exception(self.employee.id,*d.values());self.refresh()
            except Exception as e:QMessageBox.warning(self,"Schedule",str(e))
    def _delete_exception(self):
        i=self.exceptions.currentRow()
        if i<0:return
        xid=self.exceptions.item(i,0).data(Qt.ItemDataRole.UserRole)
        if QMessageBox.question(self,"Delete exception","Delete selected exception?")==QMessageBox.StandardButton.Yes:
            try:self.service.delete_exception(xid);self.refresh()
            except Exception as e:QMessageBox.warning(self,"Schedule",str(e))
