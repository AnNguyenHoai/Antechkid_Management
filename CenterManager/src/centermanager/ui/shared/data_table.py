...
class DataTable(QWidget):
    ...
    def _update_table(self) -> None:
        ...
        total_pages = (
            self._total_rows + self._page_size - 1
        ) // self._page_size
        self.page_label.setText(
            f"Page {self._current_page + 1} of {max(1, total_pages)}"
        )
    ...
    def _on_sort(self, col: int) -> None:
        if (
            ...
            and self._columns[col].get("sortable", False)
        ):
    ...
    def _on_context_menu(self, pos) -> None:
        ...
        if index.isValid():
            self.context_menu_requested.emit(
                self.table.mapToGlobal(pos), index.row()
            )
