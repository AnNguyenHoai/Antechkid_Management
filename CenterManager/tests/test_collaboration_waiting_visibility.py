...
def create_seeded_remote(tmp_path):
    ...
    manifest = {
        "schema_version": 1, "runtime_version": 1, "database_version": 1,
        "minimum_app_version": "0.1.0", "publisher": "Test", "branch": "main",
        "created_at": datetime.now().isoformat(), "published_at": None,
    }
...
def create_client(tmp_path, remote_path, name, lease_duration_seconds=None):
    ...
    subprocess.run(["git", "clone", "--branch", "main", str(remote_path), str(repo_path)], capture_output=True, check=True)
    subprocess.run(["git", "config", "user.name", f"User {name}"], cwd=repo_path, capture_output=True, check=True)
    subprocess.run(["git", "config", "user.email", f"{name}@example.com"], cwd=repo_path, capture_output=True, check=True)
    provider = GitSynchronizationProvider(repo_path=repo_path, repository_url=str(remote_path), token="", branch="main")
    if lease_duration_seconds is not None:
        provider._lease_duration_seconds = lease_duration_seconds
    provider.connect()
    event_bus = EventBus()
    cm = CollaborationManager(
        runtime_root=runtime_root, event_bus=event_bus, sync_provider=provider,
...
def get_main_status(repo_path):
    return subprocess.check_output(["git", "status", "--porcelain"], cwd=repo_path, text=True).strip()
...
def test_machine_b_sees_machine_a_lock(qapp, remote_path, tmp_path):
    ...
    try:
        ...
        # start_client() already established B's initial snapshot. An
        # unchanged poll does not emit snapshot_changed; it emits only
        # poll_completed. Therefore inspect the established snapshot instead
        # of waiting for a change that cannot occur yet.
        assert client_b["poller"].get_last_snapshot() is not None

        result_a = client_a["cm"].request_write()
        assert result_a.is_granted, "A should acquire lock"

        # A lock changes the remote snapshot, so snapshot_changed is the
        ...
        assert snapshot_b.has_lock()
        assert snapshot_b.lock_owner() == "User A"
    finally:
        try:
...
def test_expired_lease_becomes_visible_cross_machine(qapp, remote_path, tmp_path):
    ...
    try:
        ...
        lock_data["lease_expires_at"] = (datetime.now() - timedelta(seconds=1)).isoformat()
        commit_sha = provider_a._create_lock_commit_plumbing(lock_data, oid)
        assert commit_sha
        ...
        assert remote_status.get("locked") is True
        lease = remote_status.get("lease_expires_at")
        assert lease is not None and datetime.fromisoformat(lease) <= datetime.now()
        snapshot_lease = snapshot_b.remote_lock.get("lease_expires_at")
        assert snapshot_lease is not None
        assert datetime.fromisoformat(snapshot_lease) <= datetime.now()
