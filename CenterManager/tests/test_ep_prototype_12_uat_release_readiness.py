...
def test_ep_prototype_12_prior_prototype_regression_contracts_are_present():
    for number in range(1, 12):
        matches = list(DOCS.glob(f"EP-PROTOTYPE-{number:02d}_*.md"))
        test_matches = list(TESTS.glob(f"test_ep_prototype_{number:02d}_*.py"))
        assert matches, f"missing EP-PROTOTYPE-{number:02d} documentation"
        assert test_matches, f"missing EP-PROTOTYPE-{number:02d} regression test"


def test_ep_prototype_12_single_application_router_and_existing_workspace_shells():
    combined = "\n".join(_read(path) for path in (
        MAIN_WINDOW, HOME, STUDENT_SHELL, CLASS_DETAIL, SESSION_DETAIL, FINANCE_SHELL
    ))
