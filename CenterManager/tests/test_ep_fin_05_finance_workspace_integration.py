...
def test_finance_workspace_owns_the_single_month_year_selector():
    ...
    assert "self.month_combo = QComboBox()" in shell
    assert "self.year_combo = QComboBox()" in shell
    assert "def _resolve_shared_period" in shell
    assert "get_period_bounds" in shell
...
def test_shell_propagates_same_canonical_period_context_to_finance_pages():
    ...
    assert "target_date, period_start, period_end, period_configured" in shell
    assert "page.refresh(" in shell
...
def test_settlement_uses_workspace_target_date_for_preview_and_writes():
    ...
    assert "self._target_date = date.today()" in settlement
