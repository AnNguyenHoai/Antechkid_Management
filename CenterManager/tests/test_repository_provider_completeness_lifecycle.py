...
EXPECTED_FACTORIES = {
    "audit_logs", "class_timeline", "attendance", "enrollments", "enrollment_freezes", "enrollment_transfers", "sessions", "session_notes",
    ...
    "teachers", "teacher_assignments", "teacher_documents", "teacher_timeline", "notes", "parents",
    "documents", "student_highlights", "finance_periods", "financial_settlements", "timeline",
}


def _public_factory_names(provider_type: type) -> set[str]:
    return {
        name for name, member in inspect.getmembers(provider_type, predicate=inspect.isfunction)
...
def _bound_session(repository):
    ...
    raise AssertionError(f"{type(repository).__name__} does not expose its bound session")
